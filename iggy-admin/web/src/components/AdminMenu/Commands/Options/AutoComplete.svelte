<!--
    Credit to https://github.com/pstanoev/simple-svelte-autocomplete/
 -->
<script lang="ts">
    //@ts-nocheck
    ...
    export let labelFieldName: string | undefined = undefined;
    export let keywordsFieldName: string | undefined = labelFieldName;
    ...
    // selected item state
    export let selectedItem: unknown = multiple ? [] : undefined;
    ...
    $: hasSelection =
        (multiple && selectedItem && selectedItem.length > 0) ||
    ...
    function processListItems(textFiltered = "") {
        ...
        const textFilteredWithoutAccents = ignoreAccents
            ...
            : textFiltered;
        const searchWords = textFilteredWithoutAccents
            .split(/\s+/g)
            .filter((word) => word !== "");
    ...
    function setScrollAwareListPosition() {
        ...
        const { bottom: inputButtom, height: inputHeight } =
...
<div
    class="{className
        ? className
        : ''} autocomplete select is-fullwidth w-full {uniqueId}"
...
>
    ...
    <div class="input-container" bind:this={inputContainer}>
        ...
        <input
            ...
            class="{inputClassName ? inputClassName : ''} {noInputStyles
                ? ''
                : 'input autocomplete-input bg-neutral-700'}"
            id={inputId ? inputId : ""}
    ...
    <div
        class="{dropdownClassName
            ? dropdownClassName
            : ''} autocomplete-list bg-neutral-700 {showList ? '' : 'hidden'}
    is-fullwidth"
        bind:this={list}
    >
...
<svelte:window
    ...
    on:scroll={() => (setPositionOnNextUpdate = true)}
/>
